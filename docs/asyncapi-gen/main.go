...
func main() {
	...
	conf, err := util.LoadConfig(*configLocation)
	if err != nil {
		log.Fatal("ERROR: unable to load config", err)
	}

	asyncAPI := spec.AsyncAPI{}
	asyncAPI.Info.Title = "External-Task-Worker"
	...
	mustNotFail(reflector.AddChannel(asyncapi.ChannelInfo{
		...
		Subscribe: &asyncapi.MessageSample{
			...
			MessageSample: new(messages.ProtocolMsg),
		},
	}))

	mustNotFail(reflector.AddChannel(asyncapi.ChannelInfo{
		Name: conf.KafkaIncidentTopic,
		Subscribe: &asyncapi.MessageSample{
			MessageEntity: spec.MessageEntity{
				Deprecated: true,
			...
			MessageSample: new(messages.KafkaIncidentsCommand),
		},
	}))

	mustNotFail(reflector.AddChannel(asyncapi.ChannelInfo{
		Name: conf.ResponseTopic,
